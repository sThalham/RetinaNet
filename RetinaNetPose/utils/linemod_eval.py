...
def evaluate_linemod(generator, model, threshold=0.05):
    ...
    for index in progressbar.progressbar(range(generator.size()), prefix='COCO evaluation: '):
        ...
        # run network
        boxes, xys, deps, quats, scores, labels = model.predict_on_batch(np.expand_dims(image, axis=0))
        #print('boxes: ', boxes)
        #print('poses: ', poses)
        #print('scores: ', scores)
        #print('labels: ', labels)
        ...
        for box, xy, dep, quat, score, label in zip(boxes[0], xys[0], deps[0], quats[0], scores[0], labels[0]):
            ...
            image_result = {
                ...
                'score'       : float(score),
                'bbox'        : box.tolist(),
                'pose'        : xy.tolist() + dep.tolist() + quat.tolist()
            }
